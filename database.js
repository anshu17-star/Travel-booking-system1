...
db.serialize(() => {
  ...
  db.get("SELECT COUNT(*) AS count FROM destinations", (err, row) => {
    if (row && row.count === 0) {
      ...
      // Ensure existing records in travel.db are updated to the local images
      db.run("UPDATE destinations SET image = '/images/golden_temple.png' WHERE title LIKE '%Golden Temple%'");
      db.run("UPDATE destinations SET image = '/images/hampi_ruins.png' WHERE title LIKE '%Hampi%'");
      db.run("UPDATE destinations SET image = '/images/meghalaya_bridges.png' WHERE title LIKE '%Meghalaya%'");
    }
  });
});
